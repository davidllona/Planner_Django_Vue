   ...
   getAveragePeriods(periodsData) {
     axios
       ...
       .then((response) => {
         ...
         this.drawChart(periodsData, averagePeriod);
       })
       .catch((error) => {
         console.error("Error al obtener el promedio de períodos:", error);
       });
   },
   drawChart(periodsData, averagePeriod) {
    const width = 800;
    const height = 600;
    ...
    chart
     ...
     .attr("x", 0)
     .attr("y", yScale(averagePeriod)) // Posición vertical según el promedio de períodos
